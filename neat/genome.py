...
class AutoencoderGenomeConfig(object):
    ...
    def __init__(self, params):
        ...
        self._params = [
            ...
            ConfigParameter('bottleneck_size', int),
        ...
        self.encoder_input_keys = [-i - 1 for i in range(self.num_inputs)]
        self.encoder_output_keys = [i for i in range(self.bottleneck_size)]

        self.decoder_input_keys = [-i - 1 for i in range(self.bottleneck_size)]
        self.decoder_output_keys = [i for i in range(self.num_inputs)]
...
class DefaultGenome(object):
    ...
    @staticmethod
    def distance(first, second, config):
        """
        ...
        node_distance = 0.0
        if first.nodes or second.nodes:
            disjoint_nodes = 0
            for k2 in second.nodes:
                if k2 not in first.nodes:
                    disjoint_nodes += 1

            for k1, n1 in first.nodes.items():
                n2 = second.nodes.get(k1)
                if n2 is None:
                    disjoint_nodes += 1
                else:
            ...
            max_nodes = max(len(first.nodes), len(second.nodes))
        ...
        if first.connections or second.connections:
            disjoint_connections = 0
            for k2 in second.connections:
                if k2 not in first.connections:
                    disjoint_connections += 1

            for k1, c1 in first.connections.items():
                c2 = second.connections.get(k1)
                if c2 is None:
                    disjoint_connections += 1
                else:
                    # Homologous genes compute their own distance value.
            ...
            max_conn = max(len(first.connections), len(second.connections))
...
class AutoencoderGenome(DefaultGenome):
   
    def __init__(self, key):

       self.key = key
       self.encoder = DefaultGenome(None)
       self.decoder = DefaultGenome(None)
       self.fitness = None
    ...
    def parse_config(cls, param_dict):
        param_dict['node_gene_type'] = DefaultNodeGene
        param_dict['connection_gene_type'] = DefaultConnectionGene
        return AutoencoderGenomeConfig(param_dict)
    ...
    def configure_crossover(self, genome1, genome2, config):
        ...
        #----------------------------------------------------------------
        # encoder
        for key, cg1 in parent1.encoder.connections.items():
            cg2 = parent2.encoder.connections.get(key)
            if cg2 is None:
                self.encoder.connections[key] = cg1.copy()
            else:
                self.encoder.connections[key] = cg1.crossover(cg2)

        # decoder
        for key, cg1 in parent1.decoder.connections.items():
            cg2 = parent2.decoder.connections.get(key)
            if cg2 is None:
                self.decoder.connections[key] = cg1.copy()
            else:
        ...
        parent2_encoder_set = parent2.encoder.nodes
        parent2_decoder_set = parent2.decoder.nodes

        # encoder
        for key, ng1 in parent1_encoder_set.items():
            ng2 = parent2_encoder_set.get(key)
            assert key not in self.encoder.nodes
            if ng2 is None:
                self.encoder.nodes[key] = ng1.copy()
            else:
                self.encoder.nodes[key] = ng1.crossover(ng2)
            
        # decoder
        for key, ng1 in parent1_decoder_set.items():
            ng2 = parent2_decoder_set.get(key)
            assert key not in self.decoder.nodes
            if ng2 is None:
                self.decoder.nodes[key] = ng1.copy()
            else:
                self.decoder.nodes[key] = ng1.crossover(ng2)
        #----------------------------------------------------------------
    ...
    def mutate(self, config):
        ...
        # Mutate connection genes.
        # Encoder
        for cg in self.encoder.connections.values():
            cg.mutate(config)

        # Decoder
        for cg in self.decoder.connections.values():
            cg.mutate(config)

        # Mutate node genes (bias, response, etc.).
        # Encoder
        for ng in self.encoder.nodes.values():
            ng.mutate(config)

        # Decoder
        for ng in self.decoder.nodes.values():
            ng.mutate(config)

    def mutate_add_node(self, config):
        if not self.encoder.connections:
            if config.check_structural_mutation_surer():
                self.mutate_add_connection(config)
            return

        if not self.decoder.connections:
            if config.check_structural_mutation_surer():
                self.mutate_add_connection(config)
            return
    ...
    def add_connection(self, config, input_key, output_key, side, weight, enabled):
        ...
        connection.weight = weight
        connection.enabled = enabled
        side.connections[key] = connection

    def mutate_add_connection(self, config):
        side = choice(['encoder', 'decoder'])
        if side == 'encoder':
            possible_outputs = list(self.encoder.nodes)
            out_node = choice(possible_outputs)

            possible_inputs = possible_outputs + config.encoder_input_keys
            in_node = choice(possible_inputs)

            # Don't duplicate connections.
            key = (in_node, out_node)
            if key in self.encoder.connections:
                # TODO: Should this be using mutation to/from rates? Hairy to configure...
                if config.check_structural_mutation_surer():
                    self.encoder.connections[key].enabled = True
                return

            # Don't allow connections between two output nodes
            if in_node in config.encoder_output_keys and out_node in config.encoder_output_keys:
                return
            ...
            # For feed-forward networks, avoid creating cycles.
            if config.feed_forward and creates_cycle(list(self.encoder.connections), key):
                return

            cg = self.create_connection(config, in_node, out_node)
            self.encoder.connections[cg.key] = cg

        else:
            possible_outputs = list(self.decoder.nodes)
            out_node = choice(possible_outputs)

            possible_inputs = possible_outputs + config.decoder_input_keys
            in_node = choice(possible_inputs)

            # Don't duplicate connections.
            key = (in_node, out_node)
            if key in self.decoder.connections:
                # TODO: Should this be using mutation to/from rates? Hairy to configure...
                if config.check_structural_mutation_surer():
                    self.decoder.connections[key].enabled = True
                return

            # Don't allow connections between two output nodes
            if in_node in config.decoder_output_keys and out_node in config.decoder_output_keys:
                return
            ...
            # For feed-forward networks, avoid creating cycles.
            if config.feed_forward and creates_cycle(list(self.decoder.connections), key):
                return

            cg = self.create_connection(config, in_node, out_node)
            self.decoder.connections[cg.key] = cg

    def mutate_delete_node(self, config):
        side = choice(['encoder', 'decoder'])
        if side == 'encoder':
            # Do nothing if there are no non-output nodes.
            available_nodes = [k for k in self.encoder.nodes if k not in config.encoder_output_keys]
            if not available_nodes:
                return -1
            ...
            connections_to_delete = set()
            for k, v in self.encoder.connections.items():
                if del_key in v.key:
                    connections_to_delete.add(v.key)

            for key in connections_to_delete:
                del self.encoder.connections[key]

            del self.encoder.nodes[del_key]

            return del_key

        else:
            # Do nothing if there are no non-output nodes.
            available_nodes = [k for k in self.decoder.nodes if k not in config.decoder_output_keys]
            if not available_nodes:
                return -1
            ...
            connections_to_delete = set()
            for k, v in self.decoder.connections.items():
                if del_key in v.key:
                    connections_to_delete.add(v.key)

            for key in connections_to_delete:
                del self.decoder.connections[key]

            del self.decoder.nodes[del_key]
    ...
    def mutate_delete_connection(self):
        side = choice(['encoder', 'decoder'])
        if side == 'encoder':
            if self.encoder.connections:
                key = choice(list(self.encoder.connections.keys()))
                del self.encoder.connections[key]
        else:
            if self.decoder.connections:
                key = choice(list(self.decoder.connections.keys()))
                del self.decoder.connections[key]

    @staticmethod
    def distance(first, second, config):
        encoder_distance = DefaultGenome.distance(first.encoder, second.encoder, config)
        decoder_distance = DefaultGenome.distance(first.decoder, second.decoder, config)
        return encoder_distance + decoder_distance

    def size(self):
        encoder_enabled = sum([1 for cg in self.encoder.connections.values() if cg.enabled])
        decoder_enabled = sum([1 for cg in self.decoder.connections.values() if cg.enabled])
        encoder_nodes = len(self.encoder.nodes)
        decoder_nodes = len(self.decoder.nodes)
        return encoder_nodes + decoder_nodes, encoder_enabled + decoder_enabled

    def __str__(self):
        s = "Key: {0}\nFitness: {1}\nEncoder nodes:".format(self.key, self.fitness)

        for k, ng in self.encoder.nodes.items():
            s += "\n\t{0} {1!s}".format(k, ng)
        s += "\nEncoder connections:"
        connections = list(self.encoder.connections.values())
        connections.sort()
        for c in connections:
            s += "\n\t" + str(c)

        s += '\nDecoder nodes:'
        for k, ng in self.decoder.nodes.items():
            s += "\n\t{0} {1!s}".format(k, ng)
        s += "\nDecoder connections:"
        connections = list(self.decoder.connections.values())
        connections.sort()
        for c in connections:
            s += "\n\t" + str(c)
    ...
    def compute_full_connections(self, config, direct):
        encoder_hidden = [i for i in self.encoder.nodes if i not in config.encoder_output_keys]
        encoder_output = [i for i in self.encoder.nodes if i in config.encoder_output_keys]
        encoder_connections = []

        decoder_hidden = [i for i in self.decoder.nodes if i not in config.decoder_output_keys]
        decoder_output = [i for i in self.decoder.nodes if i in config.decoder_output_keys]
        decoder_connections = []
        ...
        if decoder_hidden:
            for input_id in config.decoder_input_keys:
                for h in decoder_hidden:
                    decoder_connections.append((input_id, h))
            for h in decoder_hidden:
                for output_id in decoder_output:
                    decoder_connections.append((h, output_id))

        if direct or (not decoder_hidden):
            for input_id in config.decoder_input_keys:
                for output_id in decoder_output:
                    decoder_connections.append((input_id, output_id))

        return encoder_connections, decoder_connections

    def connect_full_nodirect(self, config):
        encoder_connections, decoder_connections = self.compute_full_connections(config, False)

        for input_id, output_id in encoder_connections:
            connection = self.create_connection(config, input_id, output_id)
            self.encoder.connections[connection.key] = connection

        for input_id, output_id in decoder_connections:
            connection = self.create_connection(config, input_id, output_id)
            self.decoder.connections[connection.key] = connection

    def connect_full_direct(self, config):
        encoder_connections, decoder_connections = self.compute_full_connections(config, True)
    ...
    def connect_partial_nodirect(self, config):
        ...
        for input_id, output_id in decoder_connections[:num_to_add]:
            connection = self.create_connection(config, input_id, output_id)
            self.decoder.connections[connection.key] = connection
